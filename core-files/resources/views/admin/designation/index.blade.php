...
@section('content')
    <div class="box">
        <div class="box-header">
            <h3>
                LIST OF DESIGNATION
                <button type="button" class="btn btn-sm pull-right btn-info" data-toggle="modal" data-target="#designation-modal">
                    <i class="fa fa-plus-circle"></i> CREATE NEW
                </button>
            </h3>
        </div>
        <div class="box-body">
            <table id="designations-table" class="table table-bordered table-striped">
                <thead>
                <tr>
                    <th>SL</th>
                    <th>Name</th>
                ...
                @foreach($designations as $k => $designation)
                    <tr>
                        ...
                        <td class="text-right">
                            ...
                            <button data-url="{{ route('delete-designation',['company_id' => Request::segment(2),'id' => $designation->id]) }}" class="btn btn-danger btn-xs flat btn-delete" data-toggle="modal" data-target="#delete-content-modal"><i class="fa fa-trash-o"></i>Delete</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    <div class="modal fade in" id="designation-modal" tabindex="-1" role="dialog" aria-labelledby="designation">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('create-designation',['company_id' => Request::segment(2)]) }}" method="post">
                    {{ csrf_field() }}
...
@section('script')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var url = "{{ route('designation',['company_id' => Request::segment(2),'id' => '']) }}/";
            ...
            $(document).on('click','.btn-edit',function () {
                ...
                $.ajax({url: api_url, success: function(result){
                        ...
                        $("#edit-designation-form input[name='name']").val(result.name);
                    }});
            });
